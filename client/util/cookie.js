...
export function parse (str) {
    ...
    parsedCookie.key   = SharedUtil.trim(parsedCookie.key);
    parsedCookie.value = SharedUtil.trim(parsedCookie.value);

    if (firstSemicolonIdx === -1)
        return parsedCookie;

    var attributesString = SharedUtil.trim(str.slice(firstSemicolonIdx).replace(/^\s*;\s*/, ''));
    ...
    while (attrValStrings.length) {
        ...
        if (separatorIdx === -1)
            ...
            value = SharedUtil.trim(attrValueStr.substr(separatorIdx + 1));
        }

        key = SharedUtil.trim(key.toLowerCase());
        ...
        switch (key) {
            ...
            case 'domain':
                //NOTE: remove leading '.'
                parsedCookie.domain = SharedUtil.trim(value.replace(/^\./, ''));
                break;
...
export function get (document, name) {
    ...
    for (var i = 0; i < cookies.length; i++) {
        var cookie = SharedUtil.trim(cookies[i]);
