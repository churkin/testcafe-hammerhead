...
export function isHammerheadAttr (attr) {
    return attr === SharedConst.HOVER_PSEUDO_CLASS_ATTR ||
           attr.indexOf(SharedConst.DOM_SANDBOX_STORED_ATTR_POSTFIX) !== -1;
}
...
export function isShadowUIElement (element) {
    while (element) {
        ...
        if (typeof element.className === 'string' &&
            element.className.indexOf(SharedConst.SHADOW_UI_CLASSNAME_POSTFIX) > -1)
            return true;
    ...
    if (el && el.classList) {
        ...
        var addClassFallback = function (el, className) {
            if (className) {
                ...
                el.className = SharedUtil.trim(setClass);
            }
        };
    ...
    if (el.classList) {
        ...
        var removeClassFallback = function (el, className) {
            if (el.className && className) {
                ...
                el.className = SharedUtil.trim(className);
            }
        };
